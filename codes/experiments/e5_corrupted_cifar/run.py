"""E5 — Corrupted-CIFAR-10 sweep (vanilla / fairds-1 / fairds-2 / ren2018).

A color-independent, texture-corruption spurious benchmark — the sibling of
Colored MNIST. Same protocol: anchor (used inside algorithms) vs held-out
val_eval vs OOD test (spurious flipped). Headline metric is OOD worst-group
(spurious-flipped) test accuracy.

Architecture: small 2-conv CNN trained from scratch.
"""
...
def evaluate(model, X, y, g, device, batch_size=512):
    model.eval()
    preds = []
    with torch.no_grad():
...
def run_one(*, method: str, seed: int, epochs: int, batch_size: int, lr: float,
    ...
    set_seed(seed)
    cfg = CorruptedCIFARConfig(n_train=n_train, majority_ratio=majority_ratio,
                               p_corr_majority=p_corr_majority, seed=seed)
    b = make_corrupted_cifar(cfg)
...
def main():
    ...
    p.add_argument("--out-root", type=str, default="results/e5")
    args = p.parse_args()
            ...
            for strength in args.strengths:
                for seed in args.seeds:
                    ...
                    try:
                        ...
                        runs.append(rec)
                        print(f"[{counter:>3}/{total}] {method:<10} s={strength:.2f} seed={seed} "
    ...
    print(f"[e5] {out}  ({time.time()-t0:.1f}s total)")

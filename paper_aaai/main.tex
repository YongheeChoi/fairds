...
(stronger group-label / two-stage methods exist, which we report). On tabular
demographic benchmarks (Adult, COMPAS) the gain on demographic-parity and
equalized-odds gaps is small; we characterize where the mechanism is strong
(structured bias) and weak (diffuse tabular bias).
\end{abstract}
...
\subsection{Demographic fairness, honestly}
\label{sec:demographic}
Table~\ref{tab:demo} reports DP and EO gaps on Adult and COMPAS. Here the picture
is different: Fairds keeps accuracy but barely moves the fairness gaps (e.g.\ Adult
DP $0.196{\to}0.189$, not significant), and the bi-level reweighter is fairer at a
small accuracy cost. The reason is structural: tabular bias is \emph{diffuse}
(spread over many weakly informative features), so the per-example reweighting
signal is small. The mechanism needs a \emph{structured} bias --- one dominant
biased attribute --- to fire. We report this boundary rather than hide it. Concretely, the per-example
weight spread on Adult is far smaller than on the image regimes; the signal still
exists (Fairds down-weights the majority group on average) but is too weak to move
DP/EO. The scope condition is therefore clear: the closed-form Shapley reweighter
is a fairness tool for \emph{structured} bias --- one dominant biased attribute ---
not a general tabular debiaser.
...
\begin{tabular}{llccc}
\toprule
data & method & DP $\downarrow$ & EO $\downarrow$ & acc \\
\midrule
Adult  & Vanilla          & 0.196 & 0.117 & 0.837 \\
       & \textit{Fairds-2}& 0.189 & 0.127 & 0.835 \\
       & Ren2018          & \textbf{0.175} & \textbf{0.102} & 0.819 \\
\midrule
COMPAS & Vanilla          & 0.240 & \textbf{0.226} & 0.667 \\
       & \textit{Fairds-2}& 0.239 & 0.244 & 0.671 \\
       & Ren2018          & \textbf{0.231} & 0.241 & 0.681 \\
\bottomrule
\end{tabular}
\caption{\textbf{Demographic fairness (lower DP/EO is fairer).} On diffuse tabular
bias, Fairds preserves accuracy but barely improves fairness; the bi-level
baseline is fairer. This is the weak-bias boundary of the mechanism.}
\label{tab:demo}
\end{table}
